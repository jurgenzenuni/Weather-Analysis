...
path_to_folder = 'x:/coding/python/Weather-Analysis-master/'
file_name = '3308704.csv'
df_weather = pd.read_csv(path_to_folder + file_name)

path_to_folder2 = 'x:/coding/python/Weather-Analysis-master/'
...
df_numacc['DATE'] = pd.to_datetime(df_numacc['DATE'])
df_resampled = df_numacc.resample('M', on='DATE').sum()  # Resample by month
plt.plot(df_resampled.index, df_resampled['# OF ACCIDENTS'])

# Use a bar plot to visualize trends
plt.plot(df_resampled.index, df_resampled['# OF ACCIDENTS'], color = '#D8BFD8')
plt.title('Monthly Number of Accidents in Manhattan (2019-2022)')
plt.xlabel('Date')
...
plt.pie(values, labels=labels, autopct='%1.1f%%', colors= ['#FFB347', '#56B4E9'])
...
plt.bar(weather_types, num_accidents_by_weather, color=['#FFB347', '#FFB6C1', '#D8BFD8', '#AEC6CF'], edgecolor='black')
...
plt.barh(['fog days','No Fog Days'], [fog_days_accidents, no_fog_days_accidents], edgecolor='black', color=['#FFB347', '#AEC6CF'])
plt.title('Average Number of Accidents by fog Conditions')
plt.xlabel('Average # of Accidents')
plt.ylabel('Fog Conditions')
...
plt.bar(['> 5.0 inches','Snowy Days', 'Non-Snowy Days', ' highest snow day, 2/1/21 (14.8inches)'], [snow2_avg_accidents ,snow_avg_accidents, no_snow_avg_accidents, feb_1_avg_accidents], color=['#D8BFD8', '#AEC6CF', '#FFB347', '#FFB6C1'], edgecolor='black')
...
plt.bar(['rain > 1.5 inches','rainy days', 'Not rainy days', 'Rainiest day, 9/1/21 (7.13 inches)'], [rain2_avg_accidents,rain_avg_accidents, no_rain_avg_accidents, sept_1_avg_accidents], edgecolor='black', color=['#D8BFD8', '#AEC6CF', '#FFB347', '#FFB6C1'])
...
colors = ['#AEC6CF', '#FFB347', '#FFB6C1', '#D8BFD8','#77DD77', 'lightyellow']
